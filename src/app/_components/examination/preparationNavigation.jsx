...
export const PreparationNavigation = () => {
    ...
    return (
        <nav className="flex text-xl border-2 border-b-0 border-[#F2F4F7] bg-white rounded-t-[24px]">
            <Link href={`/examinations/${id}/study-materials`}
                className={`w-auto cursor-pointer ${pathName === `/examinations/${id}/study-materials` ? `${activeStyles}` : `${baseStyles}`}`}
            >
                Study Materials
            </Link>
            <Link href={`/examinations/${id}/practice-test`}
                className={`w-auto cursor-pointer ${pathName === `/examinations/${id}/practice-test` ? `${activeStyles}` : `${baseStyles}`}`}
            >
                Practice Test
            </Link>
        </nav>
