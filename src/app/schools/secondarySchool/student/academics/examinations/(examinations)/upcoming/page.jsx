...
import { examButton, examCard, examStatus, examTitle } from "../../../../../../../../../lib/cardStyles"
...
const UpcomingExams = () => {
    ...
    const url = `${process.env.NEXT_PUBLIC_BASE_URL}examination?filter=upcoming`
    const token = process.env.NEXT_PUBLIC_TOKEN
    const cacheId = "upcoming exams"
    const {data: upcomingExams} = useFetchData(url, getOptions(token) , cacheId)
                ...
                <section className="w-full grid grid-cols-3 gap-[48px] ">
                {
                    upcomingExams?.map((data) => 
                        <Card cardStyle={examCard} key={data?.id}>
                            ...
                            <Button onClick={() => router.push(`/examinations/${data?.id}/study-materials`)}
