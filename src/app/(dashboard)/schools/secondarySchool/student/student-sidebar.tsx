...
function isActive(pathname: string, href: string) {
  ...
  // Dashboard should only be active on the exact dashboard page
  if (target === normalize(BASE)) {
    return pathname === target;
  }

  // Other items stay active for nested routes
        ...
        return (
          <Link
            ...
            className={[
              ...
              active
                ? "py-3 bg-white text-[#175F89] shadow-sm"
...
export default function StudentSidebar() {
  return (
    <aside className="hidden md:block fixed inset-y-0 left-0 w-56 lg:w-64 bg-[#216388]">
      <div className="flex h-full flex-col">
        <div className="px-5 lg:px-6 pt-4 pb-3">
          <Image
            src="/_assets/brand-logo.png"
            alt="NetzerTech"
            width={340}
            height={102}
      ...
      >
        <div className="flex h-full flex-col px-4 pb-2">
          <div className="flex items-center justify-between pb-3">
            ...
            <button
              onClick={onClose}
              className="rounded-md p-2 hover:bg-white/10 shrink-0"
            >
              <X className="h-5 w-5 text-white" />
            </button>
          </div>

          <div className="-mt-13 flex flex-1 flex-col gap-3 pt-1">
