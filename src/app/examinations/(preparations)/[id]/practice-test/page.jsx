...
const PracticeTest = () => {
    ...
    const cacheId = "practice tests"
    const { data: testData } = useFetchDataWithId(url, getOptions(token), id, cacheId)

    const handleClick = (id) => {
        const url = `${process.env.NEXT_PUBLIC_BASE_URL}examination/${id}`
        const { data: start } = useFetchDataWithId(url, getOptions(token), id, cacheId)
        if (start?.startTime) {
            router.push(`/examinations/${id}/test`)
        }
                    ...
                    <Card key={data?.id}
                        ...
                        <div className="flex flex-col gap-y-[36px] text-white font-semibold">
                            ...
                            <Button onClick={() => handleClick(data?.id)}
                                buttonStyle={"flex justify-center items-center h-[46px] py-[16px] px-[24px] bg-[#216388] hover:bg-[#9FCAE2] active:bg-[#216388] rounded-[10px] "}
                            >
